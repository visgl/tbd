...
attributes, WebGPU storage bindings, and model-specific conversion helpers.
...
Arrow adapters publish `GPUVector.format` strings from
[`@luma.gl/gpgpu/gpu-data`](/docs/api-reference/gpgpu/gpu-vector-format). Arrow
`DataType` values are retained only as adapter/readback metadata during
migration.
...
`vertex-list<...>` vectors keep source row boundaries through list-offset
...
| `Float64` | 🟡 | ✅ | `convertArrowToGPUVector()` lowers to chunk-preserving nullable `Float32` by default and reports precision loss in its plan. WebGPU uploads binary64 bytes unchanged and runs one `fround()` pass; the prepared result exposes zero-copy high and residual-low Float32 views. Other devices use CPU fallback. Semantic paths may instead use origin-relative values or raw word pairs. | No generic `f64` vertex attribute or WGSL scalar path; use the high view for ordinary Float32 consumers, high/low views for fp64-style arithmetic, or custom word-pair helpers. | Generic planned conversion, matrix truncation, and per-row Float32 path deltas plus retained origins. |
| `Int64 \| Uint64` | 🟡 | ✅ | `convertArrowToGPUVector()` lowers generic values to nullable `Float32` by default or to an explicitly requested compatible numeric shape, with a precision warning. | Use prepared 32-bit values unless an application-specific word-pair shader preserves integer precision. | Generic planned conversion; DGGS uses the specialized exact Uint64 path below. |
...
| `Bool` | 🟡 | ✅ | Arrow Bool values are bit-packed; `convertArrowToGPUVector()` repacks them to chunk-preserving nullable `Uint8` rows. | Integer 0/1 attributes or storage values after planned repacking. | Universal planned conversion and `closeArrowPaths()` CPU closed-path flags. |

### Vector-valued Columns

| Arrow data type | GPU<br/>upload | nullable | Upload notes | Shader-facing<br/>use | Higher-level<br/>model support |
| --- | :---: | :---: | --- | --- | --- |
| `FixedSizeList<numeric,`<br/>`1 \| 2 \| 3 \| 4>` | ✅ | ✅ | Directly supported 8/16/32-bit rows preserve parent and child validity. The universal planner lowers 64-bit leaves to the requested 32-bit shape while preserving chunks and nulls; Float64 uses GPU high/low splitting on WebGPU.<br/>Semantic color conversion normalizes `Uint8`, `Float16`, and `Float32` RGB/RGBA rows into RGBA8. | Vector attributes or storage rows; consuming models must explicitly honor validity, and 3-component 8-bit and 16-bit integer formats are WebGL-only unless padded or normalized through the color adapter. | Positions, point rows, normalized colors, scene-linear/HDR colors, pixel offsets, clip rectangles, mesh attributes, and path style rows. |
...
The [Points example](/examples/arrow/arrow-points) demonstrates `FixedSizeList<Float32, 2 | 3 | 4>`
and DenseUnion point rows, including M-coordinate or timestamp animation and hover
identity labels. The [Lines example](/examples/arrow/arrow-lines) demonstrates
DenseUnion line rows and polygon outlines by normalizing them into prepared line rows
before the path models consume them. The [GeoArrow example](/examples/arrow/arrow-geoarrow)
routes a mixed DenseUnion geometry column through point, line, and polygon renderers.

`@luma.gl/arrow` adapts Apache Arrow buffers to the runtime-neutral descriptors from
`@math.gl/geoarrow`. `makeGeoArrowColumnFromArrowVector()` borrows the source buffers, while
interleaving, WKB/WKT decoding, dense-union normalization, and polygon tessellation delegate to
math.gl. Renderer-specific color expansion and GPU resource creation remain in luma.gl.
...
The [Time Columns example](/examples/arrow/arrow-time-columns) prepares aligned
`DateDay`, `TimeMillisecond`, `TimestampMillisecond`, and
`DurationMillisecond` event rows together with `convertArrowTemporalToGPUVectors()`,
then renders the same relative `Float32` columns as instanced attributes or
WebGPU storage rows. The [Blinking Stars example](/examples/arrow/arrow-temporal-starfield)
...
`List<FixedSizeList<Float32, 4>>`, DenseUnion LineString, and `List<Timestamp>`
Trips-style stream example.
...
shader interfaces consume RGBA. The semantic color planner expands
`FixedSizeList<Uint8, 3>` with alpha `255` and
`FixedSizeList<Float16 | Float32, 3>` with alpha `1.0`, then materializes
canonical `unorm8x4` bytes. Alpha values above `1.0` have no portable meaning
unless a layer explicitly defines application-specific semantics.
...
Use a multi-attribute `BufferLayout` when one logical row needs several
attribute views. `GPUInputSchema.attributeNames` can then connect that one
logical table column to every shader-visible view.
...
value type, and byte stride. `convertArrowMatrixToGPUVector()` accepts metadata-tagged
...
The Arrow helper recognizes the matrix metadata and creates one shared
`BufferLayout` with a vector attribute view for each matrix column.

## Temporal Columns

`convertArrowTemporalToGPUVector()` and `convertArrowTemporalToGPUVectors()` normalize
...
`PathTripsStorageModel`. `Interval` and `List<Interval>` are intentionally
...
`convertDggsCellIdsToGPUKeys()` accepts UTF-8 geohash, quadkey, S2, A5, or H3 IDs
and parses them on the GPU into Uint64 rows. `convertDggsCellKeysToGPUPaths()`
...
example](/examples/arrow/arrow-dggs-polygons).
...
coordinates produced from Float64 path conversion.
...
## Converting Arrow Paths for Rendering

Use `ArrowPathRenderer.convertToGPUVectors()` when raw Arrow vectors need to become
...
Path model constructors stay `GPUVector`/prepared-state only; raw Arrow path vectors
belong at the conversion boundary. For Float64 paths, shaders should transform
deltas with a zero homogeneous component and add the CPU-updated view origin
before projection.

`PathStorageModel` keeps the default storage path record compact: each
...
tables explicitly with `makeGPUTableFromArrowTable()` before constructing the model.
...
Streaming sources stay application-owned. Convert each yielded Arrow record
batch once, keep the resulting immutable `GPURecordBatch`, and add it to the
retained `GPUTable`:
...
- [ShaderLayout](https://luma.gl/docs/api-reference/core/shader-layout)
- [BufferLayout](https://luma.gl/docs/api-reference/core/buffer-layout)
- [Vertex Formats](https://luma.gl/docs/api-reference/core/vertex-formats)
