...
<ExperimentalDocsTabs active="trace-scene" />
...
[`@luma.gl/experimental/gpu-trace`](/docs/api-reference/experimental/gpu-trace), not from the generic
...
<GPUOperationContract operation="gpu-trace-scene" />
...
the same graph. [`GPUTraceInteraction`](/docs/api-reference/experimental/gpu-trace/interaction)
...
outputs with [`getGPUTracePickingShader`](https://luma.gl/docs/api-reference/experimental/gpu-trace/picking). Its visibility-aware shader uses
these same canonical records and scanned thread offsets without adding picking state to the scene.

## Current scope

This class establishes canonical ingestion, stable trace-to-scene identity, preserved source
