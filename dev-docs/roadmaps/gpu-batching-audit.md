...
Current execution, optimization, fragmentation, and streaming status lives in the
[GPU compute tranche map](./compute-roadmap.md). This audit records the batching contract and its limits.
...
GPU core execution now uses `Kernel`; see the [execution migration and baseline](compute-kernel-migration.md).
...
1. `GPUFFT2D` and the audited solver/reduction paths now compose through graph-owned execution
   (#3282, #3284). Their remaining limits are algorithmic: one complete FFT matrix must fit a
   binding, and the float32 SPD solvers have absolute tolerances and bounded iteration budgets.
...
The portable global sort computes ranks against other sorted spans. Sparse multiplication now
reuses gathered values across row blocks when profitable (#3292), while remaining gather routing
still visits nonzero/vector chunk pairs. Grid index passes visit position/cell pairs. Chunked
ancestor projection composes parent jumps in at most 32 levels of source/output chunk pairs.
Chunked search visits bounded query tiles and value chunks;
