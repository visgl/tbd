# compute Kernel execution migration

**Status:** Landed in [#3281](https://github.com/visgl/luma.gl/pull/3281).
See the [current GPU compute tranche map](./compute-roadmap.md) for delivery order.
...
## Subsequent execution work

FFT2D now composes through graph-owned resources; see the
[execution lifecycle audit](./compute-execution-lifecycle.md). The
[solver consolidation](./compute-solver-consolidation.md) subsequently landed hierarchical reductions,
chunked PCG warm starts, convergence/breakdown tests, and duplicate-path removal. Those changes are
complete. Kernel still depends on luma device, shader, pipeline, and compute-pass resources;
native WebGPU independence remains a boundary change in the [current tranche map](./compute-roadmap.md).
