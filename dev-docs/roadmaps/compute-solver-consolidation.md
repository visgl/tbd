# GPU compute solver and reduction consolidation

**Status:** Landed in [#3284](https://github.com/visgl/luma.gl/pull/3284).
See the [current GPU compute tranche map](./compute-roadmap.md) for subsequent work.
...
preconditioner interface is introduced. Subsequent [fragmentation work](./compute-fragmentation.md)
in #3292 reduces CSR routing and shared planning costs. Explicit incremental execution landed in #3294; solver-specific incremental
algorithms remain separate work. PLOOR analysis, remaining routing costs, and native WebGPU
independence are tracked in the [current tranche map](./compute-roadmap.md).
